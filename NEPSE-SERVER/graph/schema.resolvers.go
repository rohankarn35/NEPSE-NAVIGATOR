...
import (
	"context"
	"fmt"
	"nepse-sever-graphql/dbmodels"
	"nepse-sever-graphql/graph/model"
	"strings"
...
func mapIPOAlerts(alerts []dbmodels.IPOAlert) []*model.IPOAlert {
	...
	for _, alert := range alerts {
		result = append(result, &model.IPOAlert{
			UniqueSymbol:           alert.UniqueSymbol,
			...
			PricePerUnit:           alert.PricePerUnit,
			Rating:                 alert.Rating,
			Units:                  alert.Units,
			MinUnits:               alert.MinUnits,
			MaxUnits:               alert.MaxUnits,
			TotalAmount:            alert.TotalAmount,
			OpeningDateAd:          alert.OpeningDateAD,
			OpeningDateBs:          alert.OpeningDateBS,
			ClosingDateAd:          alert.ClosingDateAD,
			ClosingDateBs:          alert.ClosingDateBS,
			ClosingDateClosingTime: alert.ClosingDateClosingTime,
			Status:                 alert.Status,
		})
	}
	return result
